...
{   
    ...
    {   
		...
		if (m_CurrentScreenOverlayColor != m_TargetScreenOverlayColor)
		{
			if(fadingBack) {
				...
				if (Mathf.Abs(m_CurrentScreenOverlayColor.a - m_TargetScreenOverlayColor.a) < Mathf.Abs(m_DeltaColor.a) * Time.deltaTime || m_CurrentScreenOverlayColor.a == 1.0f)
				{
					m_CurrentScreenOverlayColor = m_TargetScreenOverlayColor;
					SetScreenOverlayColor(m_CurrentScreenOverlayColor);
					if(fadeBack) {
	...
	public void StartFade(Color newScreenOverlayColor, float fadeDuration, bool fadeBack = false)
	{
		...
		else					// initiate the fade: set the target-color and the delta-color
		{
			...
			m_DeltaColor = (m_TargetScreenOverlayColor) / fadeDuration;
		}
	}
}